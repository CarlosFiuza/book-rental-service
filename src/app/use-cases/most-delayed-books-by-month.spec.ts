...
describe('Most delayed books by month', () => {
  ...
  it('Should be able to return statistics about delayed books by month', async () => {
    ...
    const date4 = new Date('2022-03-29');
    ...
    await rentRepository.create(
      makeRent({
        replicaId: replicaId1,
        createdAt: date4,
        expectedReturnAt: new Date('2022-03-30'),
        returnedAt: new Date('2022-03-31'),
      }),
    );
    ...
    expect(rentRepository.rents).toHaveLength(10);
    ...
    expect(statistic.at(0)?.mar?.at(0)?.num).toEqual(4);
  });
});
